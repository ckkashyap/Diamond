/*
 * e1000e.c - Intel 82574L (e1000e) NIC driver
 *
 * PCI vendor/device: 0x8086 / 0x10D3
 * BAR0: 64-bit MMIO (unlike the 82540EM which uses a 32-bit BAR).
 * Register layout is compatible with e1000 for basic TX/RX operation.
 * Key differences from e1000:
 *   - BAR0 is always 64-bit; high 32 bits live at config offset 0x14.
 *   - After reset, release PHY semaphore via EXTCNF_CTRL and clear GCR bit 22.
 *   - TIPG value tuned for 82574L PHY timing.
 */
...
/* ── Register offsets (shared with e1000) ────────────────────────────────── */
...
/* 82574L-specific */
#define E1000E_GCR      0x05B00u   /* GIO Control Register */
#define E1000E_EXTCNF   0x0F00u    /* Extended Configuration Control */
...
static int e1000e_probe(uint64_t hhdm_offset, uint64_t kphys, uint64_t kvirt) {
    s_kphys = kphys;
    s_kvirt = kvirt;

    uint8_t bus, dev, fn;
    if (!pci_find(0x8086, 0x10D3, &bus, &dev, &fn)) return 0;

    uint16_t cmd = pci_read16(bus, dev, fn, 0x04);
    pci_write16(bus, dev, fn, 0x04, cmd | 0x0006);

    /* BAR0 on 82574L is always 64-bit */
    uint32_t bar0_lo = pci_read32(bus, dev, fn, 0x10);
    uint32_t bar0_hi = pci_read32(bus, dev, fn, 0x14);
    uint64_t bar_phys = ((uint64_t)bar0_hi << 32) | (bar0_lo & ~0xfU);
    ...
    /* Release PHY semaphore and clear GIO Slave Disable (GCR bit 22) */
    reg_w(E1000E_EXTCNF, 0);
    reg_w(E1000E_GCR, reg_r(E1000E_GCR) & ~(1u << 22));
    ...
    reg_w(E1000_TIPG, 0x00702008u);   /* 82574L PHY timing */
...
static void e1000e_mac(uint8_t mac[6]) {
    for (int i = 0; i < 6; i++) mac[i] = s_mac[i];
}

static int e1000e_link_up(void) {
    if (!s_ready) return 0;
    return (reg_r(E1000_STATUS) & 0x02) ? 1 : 0;
}

static int e1000e_send(const void *data, uint16_t len) {
...
static int e1000e_recv(void *buf, uint16_t maxlen, uint16_t *len_out) {
    if (!s_ready) return -1;
...
struct nic_driver e1000e_driver = {
    .name    = "e1000e",
    .probe   = e1000e_probe,
    .mac     = e1000e_mac,
